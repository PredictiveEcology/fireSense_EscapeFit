...
#    `functionName` is a function's name in your module (e.g., `fireSense_EscapeFitEvent1`).
# 2. Copy this file to the tests folder (i.e., ` # replace with empty string insteadfireSense_EscapeFit/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense_EscapeFit")
  ...
  parameters <- list(
    ...
    fireSense_EscapeFit = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSense_EscapeFitEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSense_EscapeFitEvent1(mySim)
  } else {
  ...
  if (exists("fireSense_EscapeFitEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSense_EscapeFitEvent2(mySim)
  } else {
